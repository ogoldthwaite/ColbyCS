...
class Data:
	...
	def write(self, filename, headers=[]):
		if(headers == []):
			headers = self.numeric_headerlist

		file = open(filename, "w")

		data = self.getColVals(headers)
...
class ClusterData(Data):
	def __init__(self, matrix = None, codes = None, codebook = None, K = 1, errors = None, headerlist = None):
		self.codes = codes
		...
		Data.__init__(self, headerlist=headerlist, matrix=matrix)		
		
		self.origData = copy.deepcopy(self.numeric_matrix)
