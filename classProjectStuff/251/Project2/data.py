...
class Data:
	def __init__(self, filename = None, definedtypes=1, headerlist = None, matrix = None):
		self.filename = filename
		self.raw_headerlist = headerlist
		self.datatypes = []
		self.numeric_matrix = np.matrix #Should be numpy matrix
		self.raw_matrix = np.matrix
	...
	def read(self, filename):
		...
		if(self.defined_types == True):
			...
			for line in csv_reader:
				numeric_subdata = []
				for i in line:
					if(line.index(i) in numeric_cols):
						try: 
							...
							try:
								...
								continue
				#numeric_subdata = [float(i) for i in line if line.index(i) in numeric_cols]
	...
	def getColVals(self, colheaders, rowIndexes=[]):
		first = True
		
		for header in colheaders:
			if(header in self.numeric_headerlist):
				data = [[self.get_value(header, i)] for i in range(self.get_num_points())]
	...
	def addColumn(self, header, dtype, column):
		if(not (len(column) == self.get_num_points()) ):
	...
	def get_rawValue(self, header, rowIndex): 
...
def main(argv):
    ...
    print("\nHeaders:")
    headers = dobj.get_numericHeaders()
    s = headers[0]
