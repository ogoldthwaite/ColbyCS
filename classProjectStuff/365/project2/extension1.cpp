...
int main(int argc, char *argv[]) {
	...
	cv::cvtColor(baseimg, baseimg, cv::COLOR_BGR2HSV);
	//Splitting up base image and adding them to the vector
	int prevrow = 0;
	for(int i = 0; i < 3; i++)
	{
		baseimgsects.push_back(cv::Mat(baseimg, cv::Rect(0,prevrow,baseimg.cols,baseimg.rows/3)));	
		prevrow+=baseimg.rows/3;		
	}

	//calculating base image histograms
	int channels[] = {0,1};
	const int histsize[] = {30, 40};
	float hrange[] = {0,180};
	float srange[] = {0,256};
	...
	{				
		...
		{				
			...
			cv::cvtColor(curimg, curimg, cv::COLOR_BGR2HSV);
	...
	for(int i = 0; i < 10; i++)
	{
		...
		std::string outputname = "./exten1out4/image"+s+".jpg";
...
{	
	...
	for(int i = 0; i < 3; i++)
	{
		curimgsects.push_back(cv::Mat(curimg, cv::Rect(0,prevrow,curimg.cols,curimg.rows/3)));	
